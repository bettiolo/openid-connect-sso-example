...
import accessTokens from '../db/access-tokens.js';
...
export default (issuer, privatePem, kid) => (req, res, next) => {
  ...
  authenticateClientCredentials(client_id, client_secret, (clientCredentialsErr, clientCredentialsValid) => {
    ...
    ensureAuthorizationCodeValid(code, client_id, redirect_uri, (authorizationCodeErr, authorizationCodeValid, userId) => {
      ...
      token.create((accessTokenErr, accessToken) => {
        ...
        accessTokens.save(accessToken, userId, client_id, (accessTokenSaveErr) => {
          if (accessTokenSaveErr) { return next(accessTokenSaveErr); }
